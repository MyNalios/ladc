odoo.define('aspl_gift_card_ee.giftCardRedeemPopup', function(require) {
    'use strict';
        ...
        CheckGiftCardBalance(e) {
            ...
            if (e.which == 13 && this.state.GiftCardNumber) {
                ...
                this.rpc({
                    ...
                    method: 'search_read',
                    domain: [['customer_id', '=', customer_id], ['card_no', '=', code],
                            ['expire_date', '>=', today],['issue_date', '<=', today]],
                    ...
                    if(result.length > 0){
                        ...
                        self.state.displayAmount = self.env.pos.format_currency(result[0].card_value);
                        if(result[0].customer_id[1]){
                           self.state.displayCustomer = result[0].customer_id[1];
                        }
                        if(result[0].card_value <= 0){
            ...
            var get_redeems = this.env.pos.get_order().get_redeem_giftcard();
             ...
             await this.rpc({
                ...
                if(result.length == 1){
                    ...
                    self.state.GiftCardAmount = result[0].card_value >= self.env.pos.get_order().get_due() ? self.env.pos.get_order().get_due() : result[0].card_value;
